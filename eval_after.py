...
# Defaults:
#   ADAPTER_DIR = outputs/creditizens-mangakissa-qwen-distill
...
ADAPTER_DIR = sys.argv[1] if len(sys.argv) > 1 else "outputs/creditizens-mangakissa-qwen-distill"
# our test set
...
"""
# not using have simplified and integrade to `generate_answer(..)` fn
...
def try_option_a(adapter_dir: str):
  ...
  # always read base model from adapter_config
...
def main():
  # Limit CPU threads if I want deterministic speed; optional:
  ...
  for it in items:
    ...
    ans = generate_answer(model, tok, q, max_new_tokens=256)
    formatted_distilled_q_a = f"User: {q}\nAssistant (distilled): {ans}"
    ...
    if wrong_target:
      ...
      answers_after.append("\n")
      print()

  os.makedirs("data_answers", exist_ok=True)
  with open("data_answers/answers_after.txt","w",encoding="utf-8") as f:
